    ...
    var group = {
            ...
            $('#search').DataTable({
                ...
                fnInitComplete: function () {
                    ...
                    var buttons = '<div class="col-sm-8"><div class="dt-buttons btn-group">' +
                        ...
                        '<span>创建用户组</span></button>' +
                        '<button class="btn btn-primary buttons-u2g" data-toggle="modal" data-target="#setModal" disabled>' +
                        '<span>User2Group</span></button>' +
                ...
                fnDrawCallback: function () {
                    $('.buttons-u2g').attr('disabled', 'disabled');
                },
        ...
        trSelected: function () {
            $('.content').delegate('table > tbody > tr', 'click', function () {
                ...
                if (_tr.length > 0) {
                    $('.buttons-u2g').removeAttr('disabled');
                } else {
                    $('.buttons-u2g').attr('disabled', 'disabled');
                }
            });
        ...
        user2Group: function () {
            $('.content').delegate('.buttons-u2g', 'click', function () {
                var gid = $('tr.selected').attr('data-id');
                $('#gid').val(gid);
            });
            var options = {
                type: "POST",
